...
HELPTEXT = <<HELP
Usage: find-visual-duplicates.rb <hashdb.yaml>* <dhash | idhash> [--ignore-within] [prepare]

Finds duplicates in the given hashdb files.
...
Arguments:
      hashdb.yaml: One or more files containing a hashdb generated by create-hashdb.rb.
   dhash | idhash: The algorithm to use for comparison.
                   dhash tends to give quite reasonable results with
                   only a slight overestimation.
                   idhash tends to find a lot more duplicates but also
                   more false positives.
  --ignore-within: if set, the algorithm will not compare images that are
                   within the same hashdb. This is useful if you have
                   multiple hashdbs from different sources and you want to
                   find duplicates between them.
          prepare: if given, the duplicates will be copied to a tmp folder for
                   review (/tmp/duplicates/). This is useful if you want to
                    visually inspect the duplicates before removing them.
HELP
