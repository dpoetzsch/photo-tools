...
HELPTEXT = <<HELP
Usage: find-non-duplicates.rb <folder-to-search> <duplicates-output.yaml>

Find all images that are in a folder and are not duplicates.
Here is an example that checks if there are images in example-folder that are
not yet sorted into the photos folder:
...
Arguments:
        folder-to-search: The folder to search for images that are not listed
                          in the duplicates output.
